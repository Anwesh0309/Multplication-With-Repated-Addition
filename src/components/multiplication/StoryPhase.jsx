      ...
      <div className={`story-card ${anim ? 'flipping' : ''}`}>
        ...
        <div className="story-text-section">
          ...
          <div className="story-mascot">
            ...
            <div className="speech-bubble">{s.mascotText}</div>
          </div>
        </div>
      </div>

      <div className="story-nav">
        <button className="story-nav-btn back" onClick={goPrev} disabled={slide === 0} style={{ opacity: slide === 0 ? 0.3 : 1 }}>← Back</button>
        <div className="story-dots">
          {STORY_SLIDES.map((_, i) => (
            <div key={i} className={`story-dot ${i === slide ? 'active' : i < slide ? 'completed' : ''}`} />
          ))}
        </div>
        <button className="story-nav-btn next" onClick={goNext}>
